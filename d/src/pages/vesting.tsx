...
export default function VestingPage() {
  ...
  // ── Derived Values ──────────────────────────────────────────────────────────
  const schedule = vestingSchedule as [bigint, bigint, bigint, boolean] | undefined;
  ...
  const tranches = hasAllocation && launchTime > 0 ? computeTranches(totalAllocation, claimedAmount, launchTime) : [];
                ...
                <div className="space-y-3">
                  {tranches.map((tranche) => (
                    ...
                    >
                      ...
                      >
                        {tranche.status === 'claimed' ? '✓' : tranche.status === 'available' ? '!' : String(tranche.index + 1)}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-zinc-500">
                          {tranche.unlockDate.toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                          })}
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        >
                          {tranche.status === 'claimed' ? 'Claimed' : tranche.status === 'available' ? 'Available' : 'Locked'}
                        </p>
                      </div>
                    </div>
                  ))}
